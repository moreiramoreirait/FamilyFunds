...
public class TagController {
    ...
    public ResponseEntity<List<com.familyfinance.entity.Tag>> findAll(
            @PathVariable UUID groupId,
            @AuthenticationPrincipal User currentUser) {
        return ResponseEntity.ok(tagService.findAll(groupId, currentUser));
    }

    @PostMapping
    @Operation(summary = "Criar tag")
    public ResponseEntity<com.familyfinance.entity.Tag> create(
            @PathVariable UUID groupId,
            @RequestBody TagRequest request,
            @AuthenticationPrincipal User currentUser) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(tagService.create(groupId, request.name(), request.color(), currentUser));
    }

    @PutMapping("/{tagId}")
    @Operation(summary = "Atualizar tag")
    public ResponseEntity<com.familyfinance.entity.Tag> update(
            @PathVariable UUID groupId,
            @PathVariable UUID tagId,
            @RequestBody TagRequest request,
            @AuthenticationPrincipal User currentUser) {
        return ResponseEntity.ok(tagService.update(groupId, tagId, request.name(), request.color(), currentUser));
    }
